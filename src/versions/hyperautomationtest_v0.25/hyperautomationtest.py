...
def main():
    ...
    if ( options[ 'build' ] ):
        ...
        username = 'opdapub' 
        password = 'opdapub123'
        server = 'opdadw1.rcc.org'
        service = 'opdapub'
        port = 1521
        # connect and grab oracle table
        connection = conData(username, password, server, service, port)
        table = grabData( str(options[ 'Database' ]), connection) ## raw json data turned to a list of record objects (a list within a list) ## need to add filter year 
